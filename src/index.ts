...
const raceUpdate = async (_id: BigInt, race: RacesResponse) => {
  ...
  const raceDb: Omit<Prisma.RaceCreateInput, "id"> = {
    ...
    finishedAt: race.finishTime
      ? new Date(Number(race.finishTime) * 1000)
      : null,
...
const contestantUpdate = async (
  created: boolean,
  _id: BigInt,
  contestant: ContestantsResponse
) => {
  const id = _id.toString();
  const raceDb = {
    pic: contestant.pic,
...
const betUpdate = async (
  _betId: BigInt,
  bet: RaceBetsResponse,
  result,
  txHash?
) => {
...
export const handledEvents = {
  ...
  ContestantRaceStatusChanged: async (
    _raceId: BigInt,
    _contestantId: BigInt,
    status: BigInt
  ) => {
    ...
    await prisma.race.update({
      where: { id: raceId },
      data: {
        contestantIds: status
          ? [...contestantIds, contestantId]
          : contestantIds.filter((id) => id !== contestantId),
      },
    });
  },
  async RaceCreatedWithContestants(
    _raceId: BigInt,
    _race: RacesResponse,
    _contestantIds: BigInt[]
  ) {
  ...
  async ContestantRaceStatusChangedBatch(
    _raceId: BigInt,
    _contestantIds: BigInt[],
    status: BigInt
  ) {
    ...
    const updatedContestantIds = status
      ? [...race.contestantIds, ...contestantIds]
      : race.contestantIds.filter((id) => !contestantIds.includes(id));
    await prisma.race.update({
      where: { id: raceId },
      data: {
  ...
  async LapFinished(
    ...
    for (const wonBet of wonBets) {
      const wonAmount = await contract.getLapResult(
        raceId,
        lap,
        wonBet.bettor,
        wonBet.contestantId
